...
ggplot(edbf_counts_both, aes(year, sum_wom / year_total, group=genre, color=genre)) + # add the variable 'genre' as a group and color argument
  ...
  theme_tufte(base_family = "Helvetica") 
```
#year,genre,age,and age 4-7-YilingXiong
...
edbf_term_counts_genre $raceword <- 
  as.integer(grepl("\\b(race|ethnicity|racial|ethnic|racism|racist)\\b", 
                                            x = edbf_term_counts_genre$word))

edbf_counts_children_race <- edbf_term_counts_genre %>%
...
ggplot(edbf_counts_children_race, aes(year, sum_race / year_total, group=1)) +
  geom_line() +
  xlab("Year") +
...
#both genre
edbf_counts_both_race <- edbf_term_counts_genre %>%
  group_by(year, genre) %>% 
  mutate(year_total = sum(n)) %>%
  filter(raceword==1) %>%
  summarise(sum_race = sum(n),
...
ggplot(edbf_counts_both_race, aes(year, sum_race / year_total, group=genre, color=genre)) + 
  geom_line() +
  xlab("Year") +
  ylab("% race-related words") +
  scale_y_continuous(labels = scales::percent_format(),
...
#all together
edbf_counts_race <- edbf_term_counts_genre %>%
  group_by(year) %>% 
  mutate(year_total = sum(n)) %>%
  filter(raceword==1) %>%
  summarise(sum_race = sum(n),
...
ggplot(edbf_counts_race, aes(year, sum_race / year_total)) + 
  geom_line() +
  xlab("Year") +
  ylab("% race-related words") +
  scale_y_continuous(labels = scales::percent_format(),
...
```
2. Yiling Xiong-Choose another set of terms by which to filter (e.g., race-related words) and plot their frequency over time.

```{r}
evdes_race <- edbfdata %>%
  select(year, description, title, sub_title) # but include genre variable here

evtsperyr_race <- evdes_race %>%
  mutate(obs=1) %>%
  group_by(year) %>% # group not only by year but also by genre
  summarise(sum_events = sum(obs))

head(evtsperyr_race)

hebing_evde_race<-evdes_race %>%
  mutate(wordcorpus=evdes_race$description+evdes_race$title+evdes_race$sub_title)
#:)remain error  
tidy_des_race <- evdes_race %>% 
  tolower(des1=description) 

tidy_des_race<-tidy_des_race
  unnest_tokens(word, tidy_des_race) %>%
  filter(str_detect(word, "[a-z]"))
...
edbf_term_counts_race $raceword <- as.integer(grepl("race|racist|discriminate|discrimination|equality", 
                                            x = tidy_des_race$word))
...
ggplot(edbf_counts_race, aes(year, sum_wom / year_total, group=1)) +
  geom_line() +
  xlab("Year") +
  ylab("% race-related words") +
  scale_y_continuous(labels = scales::percent_format(),
